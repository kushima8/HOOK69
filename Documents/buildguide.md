...
QMKにHOOK69のコードはまだマージされていませんのでこの[ブランチ](https://github.com/kushima8/qmk_firmware/tree/hook69)を使用してください。  
動作確認の為に[HEXファイル](https://github.com/kushima8/HOOK69/tree/main/HEX)を用意しています。  
...
## 組み立て
※Daihuku Keyboardさんによるビルド動画があります。  
...
](https://www.youtube.com/watch?v=qUh0zuOqxo4)  
※完成見本写真などではPro Microの接続位置を左端にしていますが、本機はリバーシブルPCBとなっております。  
　Pro Microを右端にして本機を使用したい場合は反転させて実装してください。  
　作成時にマスキングテープを使い表面と裏面がわかるようにしておくと作業ミスが少なくなります。  
...
* 10.キースイッチの取り付け
  ...
  * キースイッチがハマりにくい場合はヤスリなどを使いトッププレートの穴を削ってください。  
...
* 11.トッププレートの取り付け
  ...
  * ネジ穴下部にM2 6mmスペーサーを置き、上部からM2 6mmネジで固定してください。  
...
* 14.Pro Microの取り付け
  ...
  * こちらが準備しているファームウェアの場合ロータリーエンコーダの割当が空ですので、REMAPなどで何かしらのキーを割当をしてから確認を行ってください。
...
* 18.ブックエンドの取り付け
  ...
  * 付属のブックエンドを配置したい液タブ上面の形状に合わせて折り曲げます。  
